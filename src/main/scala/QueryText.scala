class QueryText {
  ...
  def getQuery6():String =
    {
      """SELECT zipcode as Zipcode, AVG(CASE grade WHEN 'A' THEN 3 WHEN 'B' THEN 2 WHEN 'C' THEN 1 END) as Grade FROM query6 WHERE grade IS NOT NULL AND zipcode IS NOT NULL GROUP BY zipcode ORDER BY zipcode"""
    }
}
